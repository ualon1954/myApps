...
function trophy() {
    ...
    if (p === 100) return ["👑", "DIAMOND", "Flawless Victory!", "Perfect score."];
    if (p >= 90) return ["💎", "PLATINUM", "Platinum Champion!", "Elite accuracy."];
    if (p >= 80) return ["🏆", "GOLD", "Gold Champion!", "Outstanding performance."];
    if (p >= 60) return ["🥈", "SILVER", "Silver Achiever!", "Great work."];
    return ["🥉", "BRONZE", "Bronze Achiever!", "Keep practicing."]
}
...
function finish() {
    ...
    $("resultTitle").textContent = correct === 10 ? "Perfect!" : correct >= 8 ? "Excellent!" : correct >= 6 ? "Great Job!" : "Keep Practicing!";
    $("summary").textContent = `You answered ${correct} of ${Q.length} correctly and earned ${score.toLocaleString()} points.`;
...
function review() {
    let r = $("reviewPanel");
    r.innerHTML = Q.map((q, n) => `<div><b>${n+1}. ${q[2]}</b><span>Correct answer: ${q[3][q[4]]}</span></div>`).join("");
